...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      double circle_center_x = cur_x + CIRCLE_OFFSETS[c] * cos(cur_yaw);  // <- Update 
      double circle_center_y =cur_y + CIRCLE_OFFSETS[c] * sin(cur_yaw);  // <- Update 
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          double dist = sqrt(
          pow(circle_center_x - actor_center_x, 2 ) + pow(circle_center_y - actor_center_y, 2 )
          );  // <- Update
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  double delta_x = main_goal.location.x - spiral[n - 1].x;  // <- Update
  double delta_y = main_goal.location.y - spiral[n - 1].y;  // <- Update
  double delta_z = main_goal.location.z - spiral[n - 1].z;  // <- Update
